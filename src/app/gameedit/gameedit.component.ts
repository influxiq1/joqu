  ...
  ngOnInit() {
    this.route.data.forEach((data) => {
      ...
      this.myForm = this.fb.group({
        ...
        st_dt: [moment(data['results'].res[0].st_dt).tz(moment.tz.guess()).toISOString(), Validators.required],
        enddt: [moment(data['results'].res[0].enddt).tz(moment.tz.guess()).toISOString(), Validators.required],
